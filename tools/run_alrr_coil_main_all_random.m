    ...
    for cluster_set_idx = 1 : length(num_clusters_set)
        ...
        for run_index = 1 : run_times
            ...
            for beta_idx = 1 : length(betas)
                ...
                dlmwrite('alrr_coil_random.txt', [lambda, beta, cluster_set_idx, run_index, acc, nmi, ari, purity, fmeasure, time_cost, iter], '-append', 'delimiter', '\t', 'newline', 'pc');
            end
        end
        for beta_idx = 1 : length(betas)
            beta = betas(beta_idx);
            ...
            dlmwrite('alrr_coil_random_avg.txt', [lambda, beta, cluster_set_idx, avg_acc, avg_nmi, avg_puritiy, avg_fmeasure, avg_ari, std_acc, std_nmi, std_puritiy, std_fmeasure, std_ari, avg_time_cost, std_time_cost], '-append', 'delimiter', '\t', 'newline', 'pc');
        end
    end
end
