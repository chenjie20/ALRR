    ...
    for lmd_idx = 1 : length(lambdas)
        ...
        for time_idx = 1 : run_times
            ...
            for beta_idx = 1 : length(betas)
                ...
                dlmwrite('alrr_usps_all_noise.txt', [rat_idx, time_idx, lambda, beta, acc, nmi, purity, fmeasure, ari, time_cost, iter], '-append', 'delimiter', '\t', 'newline', 'pc');
            end
        end
        for beta_idx = 1 : length(betas)
            beta = betas(beta_idx);
            ...
            dlmwrite('alrr_usps_avg_noise.txt', [rat_idx, lambda, beta, avg_acc, avg_nmi, avg_puritiy, avg_fmeasure, avg_ari, std_acc, std_nmi, std_puritiy, std_fmeasure, std_ari, avg_time_cost, std_time_cost], '-append', 'delimiter', '\t', 'newline', 'pc');
        end   
    end
end
