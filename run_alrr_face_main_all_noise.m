        ...
        for lmd_idx = 1 : length(lambdas)
            lambda = lambdas(lmd_idx);   
               ...
               for beta_idx = 1 : length(betas)
                ...
                disp([lambda, beta, acc, nmi, purity, fmeasure, ari, time_cost, iter]);
                dlmwrite('alrr_face_yaleb_all_noise.txt', [rat_idx, time_idx, lambda, beta, acc, nmi, purity, fmeasure, ari, time_cost, iter], '-append', 'delimiter', '\t', 'newline', 'pc');
                end
        end
    end
    for lmd_idx = 1 : length(lambdas)
        ...
        for beta_idx = 1 : length(betas)
            ...
            avg_puritiy = mean(purities(rat_idx, : , lmd_idx, beta_idx));
            std_puritiy  = std(purities(rat_idx, : , lmd_idx, beta_idx));
            avg_fmeasure = mean(fmeasures(rat_idx, : , lmd_idx, beta_idx));
            std_fmeasure  = std(fmeasures(rat_idx, : , lmd_idx, beta_idx));            
            ...
            dlmwrite('alrr_face_yaleb_all_avg_noise.txt', [rat_idx, lambda, beta, avg_acc, avg_nmi, avg_puritiy, avg_fmeasure, avg_ari, std_acc, std_nmi, std_puritiy, std_fmeasure, std_ari, avg_time_cost, std_time_cost], '-append', 'delimiter', '\t', 'newline', 'pc');
        end
    end
end
