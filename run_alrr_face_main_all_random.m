...
num_clusters_set = [10, 20,30]; %10, 20: 7,2 | 30: 7,3
run_times = 10;
[m, n] = size(X);
lambdas = [7];
betas = [2, 3];
    ...
    for cluster_set_idx = 1 : length(num_clusters_set)
        ...
        for run_index = 1 : run_times
            ...
            for beta_idx = 1 : length(betas)
                ...
                dlmwrite('alrr_face_yaleb_random_new.txt', [lambda, beta, cluster_set_idx, run_index, acc, nmi, ari, purity, fmeasure, time_cost, iter], '-append', 'delimiter', '\t', 'newline', 'pc');
            end
        end
        for beta_idx = 1 : length(betas)
            beta = betas(beta_idx);
            ...
            dlmwrite('alrr_face_yaleb_random_avg_new.txt', [lambda, beta, cluster_set_idx, avg_acc, avg_nmi, avg_puritiy, avg_fmeasure, avg_ari, std_acc, std_nmi, std_puritiy, std_fmeasure, std_ari, avg_time_cost, std_time_cost], '-append', 'delimiter', '\t', 'newline', 'pc');
        end
    end
end
